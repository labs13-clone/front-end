...
export default function TestAndSubmitChalllenge(props) {
    if (props.passed && props.testRan && props.areInputFieldsFilled) {
    ...
    } else if (props.passed && props.testRan && !props.areInputFieldsFilled) {
    ...
    } else if (!props.passed && props.testRan) {

        return <div className="test-button-container">
            <button disabled={!props.buttonState} className="run-tests" onClick={props.runTests}>{props.buttonState ? 'Run Tests' : 'Disabled'}</button>
            <h2>Looks like your code is not passing the tests!</h2>
        </div>

    } else if (!props.passed && !props.testRan && props.buttonState) {
        ...
        return <div className="test-button-container">
            <button disabled={!props.buttonState} className="run-tests" onClick={props.runTests}>{props.buttonState ? 'Run Tests' : 'Disabled'}</button>
            <h2>Please provide at least one test before we move forward.</h2>
        </div>
    }
}
